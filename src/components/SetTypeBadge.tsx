...
/** The SET column cell. Tapping opens the set-type menu. Drop sets are indented with a rule. */
export function SetTypeBadge({ type, label, onClick }: { type: SetType; label: string; onClick?: () => void }) {
  return (
    <button
      ...
      className={`w-full h-10 flex items-center tabular ${type === 'drop' ? 'justify-end' : 'justify-center'} ${TONE[type]}`}
    >
      {type === 'drop' ? <span className="w-[26px] leading-7 text-center border-l-2 border-drop">{label}</span> : label}
    </button>
  );
}
